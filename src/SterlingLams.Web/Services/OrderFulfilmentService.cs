...
public class OrderFulfilmentService : IOrderFulfilmentService
{
    ...
    public async Task<FulfilOutcome> FulfilPaidOrderAsync(int orderId)
    {
        try
        {
            ...
            // Variant label per (product, variant) from the order lines — so transfer manifests show
            // the exact colour/finish (e.g. "Silver" vs "Gold"), not just the product name.
            var variantNames = order.Items
                .Where(li => li.ProductVariantId != null)
                .GroupBy(li => (li.ProductId, li.ProductVariantId!.Value))
                .ToDictionary(g => g.Key, g => g.First().VariantName);
                ...
                else
                {
                    ...
                    foreach (var bySource in alloc.Where(kv => kv.Key.store != fulfilStore.Id).GroupBy(kv => kv.Key.store))
                    {
                        ...
                        var transfer = new StockTransfer
                        {
                            ...
                            // Auto-generated by the fulfilment engine — no staff member is the author, so
                            // leave the actor unset (shown as "System"). Attributing it to the customer
                            // (order.UserId) was wrong: they never created or approved a stock transfer.
                        ...
                        foreach (var kv in bySource)
                        {
                            ...
                            var vname = vid != null && variantNames.TryGetValue((pid, vid.Value), out var vn) ? vn : null;
                            transfer.Items.Add(new StockTransferItem
                            {
                                ProductId = pid, ProductVariantId = vid, ProductName = pname, VariantName = vname,
                                RequestedQty = qty, ApprovedQty = qty
                            });
                            var emailLabel = string.IsNullOrEmpty(vname) ? pname : $"{pname} ({vname})";
                            emailItems.Add((emailLabel, qty, HttpImg(productImages.GetValueOrDefault(pid))));
                        }

...
// Adds an eye button to every password field (storefront + all back-office forms) so people can
// reveal what they typed. Pure progressive enhancement: with no JS it's a normal password box.
// Idempotent per input; type="text" generated-password fields are left alone (they're already shown).
(function () {
    var EYE = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg>';
    ...
    function enhance(input) {
        if (!input || input.getAttribute('type') !== 'password' || input.dataset.pwReveal === '1') return;
        ...
        btn.innerHTML = EYE;
        btn.style.cssText = 'position:absolute;top:0;right:0;height:100%;width:2.75rem;display:flex;' +
...
// ─── Slide-out mini-cart drawer (opens after Add to Cart / clicking the bag) ──
(function () {
    ...
    async function refresh() {
        try {
            var res = await fetch('/Cart/MiniCart');
    ...
    body.addEventListener('click', async function (e) {
        ...
        try {
            ...
            if (data && data.success) { updateCartBadge(data.cartCount); await refresh(); }
        } catch (err) {}
    });

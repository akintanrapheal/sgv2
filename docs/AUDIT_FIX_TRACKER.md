...
| OP-47 | Checkout: `DeliveryAddress.*` are unconditionally `[Required]`, so **store-pickup orders are blocked** unless delivery-address fields are filled | variant Phase 2 | Make address required only for Delivery fulfilment. |
| OP-48 | Per-variant **stock-take** count UI still pool-level (grid editing + POS + online are per-variant) | variant Phase 2 | Add variant sub-rows to the stock-take sheet (variance cells keyed by variant). |
